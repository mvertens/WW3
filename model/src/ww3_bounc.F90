...
PROGRAM W3BOUNC
  ...
  INTEGER                 :: IX, IY, ISEA, I,JJ,IP,IP1,J,IT,       &
       ...
       IRET, ICODE, NDSL
  INTEGER :: NCID1,DIMIDLAT,DIMIDLON,DIMIDX,DIMIDY ! Line added by Clio 4 April 2026
  INTEGER :: DIMYLEN,DIMXLEN ! Line added by Clio 4 April 2026
  INTEGER :: NTMP ! Line added by Clio 4 April 2026
  INTEGER :: NTIMETOT,DIMIDT,DIMTLEN,NSTATION ! Added by Clio 4 April 2026
  ...
  REAL, ALLOCATABLE       :: SPEC2D(:,:,:,:), LATS(:), LONS(:),    &
       ...
       XBPO(:), YBPO(:), RDBPO(:,:),         &
       ABPIN(:,:), ABPIN2(:,:,:)
  REAL, ALLOCATABLE       :: SPEC2D_TMP(:,:,:,:) ! Added by Clio 4 April 2026
#ifdef W3_RTD
  ...
  DOUBLE PRECISION, ALLOCATABLE       :: TIMES_TMP(:) ! Added by Clio 4 April 2026
  !
...
400 CONTINUE
  ...
  IF ( INXOUT.EQ.'WRITE') THEN
    ...
    !
    ! for all the data in one netcdf file
    IF (NBO2.EQ.1) THEN
      WRITE(6,*) 'CASE 1'
      ...
      iret=nf90_open(path=trim(SPECFILES(IP)),mode=nf90_nowrite,ncid=ncid(IP))
      WRITE(6,*) 'Opening the only file:',trim(SPECFILES(IP))
      call check_err(iret)
      write(6,*) 'opened'
      ! dimensions
      iret=nf90_inq_dimid(ncid(IP),'time',dimid(IP,1))
      call check_err(iret)
      iret=nf90_inq_dimid(ncid(IP),'freq',dimid(IP,2))
      call check_err(iret)
      iret=nf90_inq_dimid(ncid(IP),'direction',dimid(IP,3))
      call check_err(iret)
      iret=nf90_inq_dimid(ncid(IP),'x',dimid(IP,4))
      call check_err(iret)
      ...
      NTHI=dimln(IP,3)
      NBO2=dimln(IP,4)
      WRITE (6,*)' FLAGLL = ',FLAGLL,  ' (T=Lat/Lon, F=Cartesian)'
         
      write(6,*) 'dimensions'
      write(6,*) NTI,NKI,NTHI,NBO2
      ...
      ! position variables : longitude/latitude  or x/y
      IF ( FLAGLL ) THEN
         iret=nf90_inq_varid(ncid(IP), 'latitude', varid(2))
         call check_err(iret)
         iret=nf90_get_var(ncid(IP), varid(2), lats)
         call check_err(iret)
         iret=nf90_inq_varid(ncid(IP), 'longitude', varid(3))
         call check_err(iret)
         iret=nf90_get_var(ncid(IP), varid(3), lons)
         call check_err(iret)
      ELSE
         IRET=NF90_INQ_VARID(NCID(IP), 'y2', VARID(2))        
         CALL CHECK_ERR(IRET)
         IRET=NF90_GET_VAR(NCID(IP), VARID(2), lats)
         CALL CHECK_ERR(IRET)
         IRET=NF90_INQ_VARID(NCID(IP), 'x2', VARID(3))        
         CALL CHECK_ERR(IRET)
         IRET=NF90_GET_VAR(NCID(IP), VARID(3), lons)
         CALL CHECK_ERR(IRET)     
      END IF
          
      write(6,*)'lons(1:3) =',lons(1:3)
      write(6,*)'lats(1:3) =',lats(1:3)
      ! freq and dir variables
      iret=nf90_inq_varid(ncid(IP),"freq",varid(4))
      call check_err(iret)
      ...
      iret=nf90_inq_varid(ncid(IP),"SPEC",varid(7))
      call check_err(iret)
      DO I = 1, NBO2
        iret=nf90_get_var(ncid(IP),varid(7),SPEC2D(:,:,:,I),       &
                        start=(/1,1,I,1,1/),count=(/NTHI,NKI,1,1,NTI/))
      ENDDO
      call check_err(iret)
    ...
    ELSE
      WRITE(6,*) 'CASE 2'
      ! Part added y Clio 4 April 2026
      ! If there are several files, is it because each file contains one bdry point (the default in WW3)
      ! or is it because each file contains several bdry points for consecutive dates (case for climate runs)
      ! To check this, we have to check the length of the position variables
      ! dimensions for the position variables : lon/lat or x/y
      ! Open the first file to check the dimensions
      IRET=NF90_OPEN(path=TRIM(SPECFILES(1)),mode=NF90_NOWRITE,NCID=NCID1)
      WRITE(6,*) 'Opening file:',TRIM(SPECFILES(1))
      WRITE(6,*) 'NCID1=',NCID1,'FLAGLL=',FLAGLL
      CALL CHECK_ERR(IRET)
      IRET=NF90_INQ_DIMID(NCID1,"y",DIMIDLAT)
      IF (IRET.NE.0) IRET=NF90_INQ_DIMID(NCID1,"latitude",DIMIDLAT)
      CALL CHECK_ERR(IRET)
      IRET=NF90_INQ_DIMID(NCID1,"x",DIMIDLON)
      IF (IRET.NE.0) IRET=NF90_INQ_DIMID(NCID1,"longitude",DIMIDLON)
      CALL CHECK_ERR(IRET)
      IRET=NF90_INQUIRE_DIMENSION(NCID1,DIMIDLAT,len=DIMYLEN)
      CALL CHECK_ERR(IRET)
      IRET=NF90_INQUIRE_DIMENSION(NCID1,DIMIDLON,len=DIMXLEN)
      CALL CHECK_ERR(IRET)
      IRET=NF90_INQ_DIMID(NCID1,"station",DIMIDLAT)
      IF (IRET.EQ.0) THEN
        IRET=NF90_INQUIRE_DIMENSION(NCID1,DIMIDLAT,len=DIMYLEN)
        CALL CHECK_ERR(IRET)
        DIMXLEN=DIMYLEN
      END IF
      WRITE(6,*) 'DIMYLEN=',DIMYLEN,'DIMXLEN=',DIMXLEN
      ! If both dims = 1, only one bdry point is given => default
      ! If one of the dims is greater than one, several bdry points are given
      IF ((DIMYLEN.EQ.1).AND.(DIMXLEN.EQ.1)) THEN ! Line added by Clio 4 April 2026
        WRITE(6,*) 'CASE 2A'
        ! Old part
        ...
        DO IP=1,NBO2
          ...
          ELSE
            IF (NKI.NE.NK1.OR.NTHI.NE.NTH1.OR.NT1.NE.NTI) THEN
          ...
          CALL CHECK_ERR(IRET)
          !
        END DO ! IP=1,NBO2 ! End old part
      ! Part added by Clio 4 April 2026 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ELSE IF ((DIMYLEN.GT.1).OR.(DIMXLEN.GT.1)) THEN ! if there are several bdry points in the file
        WRITE(6,*) 'CASE 2B'
        ! We need to concatenate over time
        ! Read all files to get the total number of timesteps
        NTIMETOT=0
        ALLOCATE(DIMID(NBO2,4),DIMLN(NBO2,4),NCID(NBO2))
        DO IP=1,NBO2
          ! open file
          ...
          CALL CHECK_ERR(IRET)
          ! dimensions
          IRET=NF90_INQ_DIMID(NCID(IP),'time',DIMIDT)
          CALL CHECK_ERR(IRET)
          IRET=NF90_INQUIRE_DIMENSION(NCID(IP),DIMIDT,len=DIMTLEN)
          CALL CHECK_ERR(IRET)
          NTIMETOT=NTIMETOT+DIMTLEN
        END DO
        WRITE(6,*) 'Total number of timesteps NTIMETOT=',NTIMETOT
        DO IP=1,NBO2 ! Loop over all files
          ! open file
          OPEN(NDSC,FILE=TRIM(SPECFILES(IP)),form='UNFORMATTED', &
               status='old',iostat=ICODE)
          IF (ICODE.NE.0) THEN
          ...
          IF (IP.EQ.1) THEN
            ...
            NKI=NK1
            NTHI=NTH1
            ALLOCATE(TIMES(NTIMETOT))
            ALLOCATE(FREQ(NK1),THETA(NTH1))
            ALLOCATE(SPEC2D(NTH1,NK1,NTIMETOT,NSTATION))
            ALLOCATE(ABPIN2(NK*NTH,NTIMETOT,NSTATION))
            ALLOCATE(LATS(NSTATION),LONS(NSTATION),STATION(16,NSTATION))

            ! position variables : lon/lat or x/y
            ! Here we assume and require that all files have the same number of bdry points
            IF ( FLAGLL ) THEN
              IRET=NF90_INQ_VARID(NCID(IP),'latitude',VARID(2))
              CALL CHECK_ERR(IRET)
              IRET=NF90_GET_VAR(NCID(IP),VARID(2),LATS)
            ...
            ! freq and dir variables
            ! Here we assume and require that all frequencies and directions
          ...
          ! Read time and concatenate
          IRET=NF90_INQ_DIMID(NCID(IP),'time',DIMID(IP,1))
          CALL CHECK_ERR(IRET)
          IRET=NF90_INQUIRE_DIMENSION(NCID(IP),DIMID(IP,1),len=DIMLN(IP,1))
          CALL CHECK_ERR(IRET)
          NTI=DIMLN(IP,1)
          ALLOCATE(TIMES_TMP(NTI))
          ! instanciates time
          REFDATE(:)=0.
          IRET=NF90_INQ_VARID(NCID(IP),"time",VARID(1))
          CALL CHECK_ERR(IRET)
          IRET=NF90_GET_VAR(NCID(IP), VARID(1), TIMES_TMP(:))
          CALL CHECK_ERR(IRET)
          IRET=NF90_GET_ATT(NCID(IP),VARID(1),"calendar",CALENDAR)
          IF ( IRET/=NF90_NOERR ) THEN
            WRITE(NDSE,951)
          ...
          END IF
          IRET=NF90_GET_ATT(NCID(IP),VARID(1),"units",TIMEUNITS)
          CALL U2D(TIMEUNITS,REFDATE,IERR) !!!!!!!!!!!!!!! CHECK WHAT U2D DOES
          CALL D2J(REFDATE,REFJULDAY,IERR) !!!!!!!!!!!!!!! CHECK WHAT D2J DOES
          ! Fill in array with times
          TIMES(NTMP+1:NTMP+NTI)=TIMES_TMP
          DEALLOCATE(TIMES_TMP)
          ...
          IRET=NF90_GET_ATT(NCID(IP),VARID(7),"add_offset",OFFSET)
          IF (IRET.NE.0) OFFSET=0.
          ! spec(direction, freq, station, time)
          ALLOCATE(SPEC2D_TMP(NTH1,NK1,NTI,NSTATION))
          DO I=1,NSTATION
            IRET=NF90_GET_VAR(NCID(IP),VARID(7),SPEC2D_TMP(:,:,:,I), &
                  start=(/1,1,I,1,1/),count=(/NTH1,NK1,1,1,NTI/))
          END DO
          CALL CHECK_ERR(IRET)
          ! apply scale_factor and add_offset
          IF (VARTYPE.EQ.NF90_SHORT) THEN
            WHERE(SPEC2D_TMP.NE.FILLVAL) SPEC2D_TMP=(EXP(SPEC2D_TMP*FACTOR*LOG(10.)))-1e-12
          ELSE
            WHERE(SPEC2D_TMP.NE.FILLVAL) SPEC2D_TMP=(SPEC2D_TMP*FACTOR)+OFFSET
          END IF
          ! Fill in array with times
          SPEC2D(:,:,NTMP+1:NTMP+NTI,:)=SPEC2D_TMP
        ...
        NBO2=NSTATION
        NTI=NTIMETOT
      ELSE ! Write that there is an error
        write(6,*) 'ERROR: SPECTRA FILES ARE DIFFERENT THAN THE THREE CASES'
      END IF ! several bdry points or one bdry point per file
    END IF ! one file or several files
    write(6,*) 'After opening one or more files HERE'
    WRITE(6,*) 'NBO2=',NBO2
