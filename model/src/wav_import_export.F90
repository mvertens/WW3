...
contains
  ...
  subroutine export_fields (gcomp, rc)
   ...
   !real(r8), pointer :: sw_lasl(:)
    ...
    if (state_fldchk(exportState, 'Sw_phs1_avg')) then
      ...
      !call accumulate  (dataptr, counter_phs1_avg, accum_phs1_avg, sec_next, fillvalue, PHS(:,NOSWLL))
       call accumulatehs(dataptr, counter_phs1_avg, accum_phs1_avg, sec_next, fillvalue, PHS(:,NOSWLL))
    end if
   ...
   !   call accumulate(dataptr, counter_thp0_avg, accum_thp0_avg, sec_next, fillvalue, THP0)
    ...
    ! Input zonal wind
    if (state_fldchk(exportState, 'Sw_u_avg')) then
       call state_getfldptr(exportState, 'Sw_u_avg', dataptr, rc=rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       if      (state_fldchk(importState, 'Sa_u10n')) then
         call state_getfldptr(importState,'Sa_u10n', sa_u, rc=rc)
       else if (state_fldchk(importState, 'Sa_u')) then
    ...
    if (state_fldchk(exportState, 'Sw_v_avg')) then
       call state_getfldptr(exportState, 'Sw_v_avg', dataptr, rc=rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       if      (state_fldchk(importState, 'Sa_v10n')) then
         call state_getfldptr(importState,'Sa_v10n', sa_v, rc=rc)
       else if (state_fldchk(importState, 'Sa_v')) then
    ...
    ! Langmuir number
     if (state_fldchk(exportState, 'Sw_lamult_avg')) then
       ...
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       call accumulate(dataptr, counter_lamult_avg, accum_lamult_avg, sec_next, fillvalue, LAMULT)
    end if
